...
echo "El presidente intenta aprobar la petición con request_id $MODIFY_RECORD_ERROR_REQUEST_ID"

APPROVAL=`curl --silent --location --request PUT "http://localhost:3002/api/approvals/$MODIFY_RECORD_ERROR_REQUEST_ID" \
--header 'Content-Type: application/json' \
--data-raw '{
    "approvalType": "Accept"
}'`
